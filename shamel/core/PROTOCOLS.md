...
10. **P-01.10 — Elastic ceremony tiers.** Ceremony density between fixed pipeline checkpoints scales by work-order tier (TRIVIAL/SMALL/STANDARD/LARGE, single-highest-trigger classification) per the CEO Operational Directive at `core/directives/elastic-pipeline.md` (ADR-015 — not a Constitutional Article, does not touch Articles 00-11); security-trigger domains (auth, DB/migrations, external API/webhook, crypto/secrets, payment flows) force LARGE regardless of file count. Tiers govern scaffolding inside the Law-1 flow only — they never skip, add, or reorder a checkpoint. Full tier definitions and the two hard gates (GATE-A, GATE-B) live in that directive, not here.
