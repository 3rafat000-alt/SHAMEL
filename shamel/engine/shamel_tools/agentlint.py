...
purpose: config lint in CI (EVOLUTION.md Round 2 #9) — static lint over the agent
         roster itself (frontmatter shape, RCCF body sections), a 105==105==105
          three-way parity check (`shamel/agents/` spawnables vs
...
_RCCF_SECTIONS = ("## |",)  # personality sections marked with ## |
...
def _agents_dir(root: Path) -> Path:
    return root / "shamel" / "agents"


def _rooms_dir(root: Path) -> Path:
    return root / "shamel" / "core" / "rooms"


def _registry_path(root: Path) -> Path:
    return root / "shamel" / "core" / "nexus" / "registry.yaml"


def _pins_path(root: Path) -> Path:
    return root / "shamel" / "core" / "nexus" / "agent-pins.json"
...
def lint_agents(root: str | Path | None = None) -> dict:
    """Validate every shamel/agents/*.md: frontmatter (name/description/model)
    ...
    for f in files:
        ...
        if len(sections) < 3:
            missing_str = ", ".join(_RCCF_SECTIONS)
            errors.append(f"{f.name}: body has only {len(sections)} personality section(s), "
                         f"expected 3+ ({missing_str})")
    ...
    try:
        ...
        if isinstance(data, dict):
            ...
            if isinstance(rooms, dict):
                return _registry_from_shamel_yaml(data)
...
def _registry_from_shamel_yaml(data: dict) -> dict:
    agents: dict[str, dict] = {}
    for _code, room in (data.get("rooms") or {}).items():
        ...
        for a in room.get("agents") or []:
            ...
            if not aid:
                continue
            spec = f"shamel/core/rooms/{_code}/agents/{aid}.md"
            spawnable = f"shamel/agents/{aid}.md"
            agents[aid] = {"spec": spec, "spawnable": spawnable}
...
def parity_check(root: str | Path | None = None) -> dict:
    """Count shamel/agents/*.md == count shamel/core/rooms/*/agents/*.md ==
...
def registry_consistency(root: str | Path | None = None) -> dict:
    ...
    for aid, entry in sorted(reg["agents"].items()):
        ...
        expected_spawnable = f"shamel/agents/{aid}.md"
...
def content_pins(root: str | Path | None = None, write: bool = False) -> dict:
    """SHA-256 of every shamel/agents/*.md against the pinned manifest.
