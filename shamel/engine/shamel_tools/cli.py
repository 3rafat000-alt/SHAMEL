...
cli — the `shamel` dispatcher every agent calls.
    ...
    shamel doctor                   self-check the library + governance (agent parity check)
...
Your route: {route} (shamel/core/nexus/routing.yaml).
Ticket task: {task}
Expected artifact: {expected}
Do the work loop (00-operating-system.md). Tools per your registry grant
(shamel/core/nexus/registry.yaml).
...
def _spec_for(role: str) -> str:
    return registry.spec_path(role) or f"shamel/core/rooms/<room>/agents/{role}.md (not in registry — check shamel registry)"
...
def cmd_rooms(_a) -> int:
    ...
    if not rooms:
        print("✗ registry empty/unreadable: shamel/core/nexus/registry.yaml", file=sys.stderr)
        return 2
    total = 0
...
def cmd_budget(_a) -> int:
    ...
    if not eff and not auto:
        print("✗ effort_scaling/budgeted_autonomy not found in shamel/core/nexus/routing.yaml",
              file=sys.stderr)
        return 2
...
def cmd_dispatch(a) -> int:
    ...
    try:
        ...
        route = "(set route per shamel/core/nexus/routing.yaml)"
...
def cmd_agents(a) -> int:
    """Generate spawnable shamel/agents/*.md from legal shamel/core/rooms/*/agents/*.md specs."""
    from pathlib import Path
    import shutil

    op = a.op or "build"
    root = Path(paths.repo_root())
    claude_agents = root / "shamel" / "agents"
    claude_agents.mkdir(parents=True, exist_ok=True)

    if op == "build":
        specs = sorted(root.glob("shamel/core/rooms/*/agents/*.md"))
        if not specs:
            print("✗ no agent specs found in shamel/core/rooms/*/agents/")
            return 2
        count = 0
        for spec in specs:
            agent_id = spec.stem
...
"""
            ...
            print(f"  {agent_id}")
            count += 1
        print(f"\n{count} spawnables written to shamel/agents/")
        return 0
        ...
        print(f"{len(pins)} pins written to shamel/core/nexus/pins.json")
        return 0
...
def cmd_doctor(a) -> int:
    ...
    try:
        ...
        print(f"  routing   : ✓ {nroutes} routes (shamel/core/nexus/routing.yaml)")
    except Exception as e:
        print(f"  routing   : ✗ {e}"); ok = False
    nreg = len(registry.agents())
    nrooms = len(registry.rooms())
    if nreg and nrooms:
        print(f"  registry  : ✓ {nrooms} rooms · {nreg} agents (shamel/core/nexus/registry.yaml)")
    else:
        print("  registry  : ✗ shamel/core/nexus/registry.yaml empty or unreadable"); ok = False
    print(f"  net-roles : {len(guard.NET_ALLOWED_ROLES)} agents may reach the web")
    print(f"  projects  : {len(paths.list_projects())}")

    # ── agent wiring: spawnables ↔ room specs (dual-file parity) ──
    if root:
        subs = _glob.glob(str(root / "shamel/agents/*.md"))
        specs = _glob.glob(str(root / "shamel/core/rooms/*/agents/*.md"))
        if len(subs) == len(specs):
            print(f"  agents    : ✓ {len(subs)} spawnables ↔ {len(specs)} room specs ({len(subs)}↔{len(specs)})")
        elif len(subs) == 0 or len(specs) == 0:
            print(f"  agents    : ~ agents missing (spawnables={len(subs)} specs={len(specs)})")
        else:
            print(f"  agents    : ✗ {len(subs)} spawnables ≠ {len(specs)} room specs"); ok = False

    # ── registry claims: every cited shamel/skills path must exist ───────
    # catches a skill row whose SKILL.md is missing.
    if root:
        reg = root / "shamel/core/nexus/registry.yaml"
        broken = []
        if reg.exists():
            for m in _re.findall(r'shamel/skills/[A-Za-z0-9_./-]+',
...
def cmd_new(a) -> int:
    ...
    try:
        ...
        ctx.mkdir()
        tmpl = paths.repo_root() / "shamel" / "brain" / "templates"
        if tmpl.exists():
...
def cmd_reflect(a) -> int:
    ...
    if a.prj:
        ...
        lessons_path = paths.repo_root() / "shamel" / "brain" / "org" / "LESSONS.md"
        if lessons_path.exists():
            lessons_text = lessons_path.read_text()
...
def build_parser() -> argparse.ArgumentParser:
    ...
    s = sub.add_parser("registry", help="print/query the org index (shamel/core/nexus/registry.yaml)")
